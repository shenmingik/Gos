...
#define IDT_MEM_DESC_CNT 0x81 // 目前总共支持的中断数
...
static struct gate_desc idt[IDT_MEM_DESC_CNT]; // idt是中断描述符表,本质上就是个中断门描述符数组

char *intr_name[IDT_MEM_DESC_CNT]; // 用于保存异常的名字

/********    定义中断处理程序数组    ********
 * 在kernel.S中定义的intrXXentry只是中断处理程序的入口,
 * 最终调用的是ide_table中的处理程序*/
intr_handler idt_table[IDT_MEM_DESC_CNT];
/********************************************/
extern intr_handler intr_entry_table[IDT_MEM_DESC_CNT]; // 声明引用定义在kernel.S中的中断处理函数入口数组
